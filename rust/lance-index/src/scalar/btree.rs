...
/// Records which domain a lookup file's `ids` column stores. Absent on any
/// file written before [`BTREE_ROW_ADDR_DOMAIN_VERSION`]: those store row ids
/// directly (numerically identical to row addresses when stable row ids are
/// disabled, but a different value entirely when they are enabled), and must
/// be treated as row-id domain rather than assumed to be addresses.
const ID_DOMAIN_META_KEY: &str = "id_domain";
const ID_DOMAIN_ROW_ADDR: &str = "row_addr";
...
/// before this version is row-id domain; see [`ID_DOMAIN_META_KEY`].
pub const BTREE_ROW_ADDR_DOMAIN_VERSION: u32 = 1;
const BTREE_INDEX_VERSION: u32 = BTREE_ROW_ADDR_DOMAIN_VERSION;
...
pub struct BTreeIndex {
    ...
    /// before [`BTREE_ROW_ADDR_DOMAIN_VERSION`]). Detected at load time from
    /// [`ID_DOMAIN_META_KEY`]; see [`ScalarIndex::results_are_row_addresses`].
    results_are_row_addresses: bool,
}
...
impl BTreeIndex {
    ...
    async fn load(
        ...
        // Absent on any file written before address-domain support: those
        // store row ids directly and must not be reinterpreted as addresses.
        let results_are_row_addresses = file_schema
            .metadata
            .get(ID_DOMAIN_META_KEY)
            .is_some_and(|domain| domain == ID_DOMAIN_ROW_ADDR);
    ...
    // row addresses, not row ids (see `ID_DOMAIN_META_KEY`).
...
pub async fn train_btree_index(
    ...
    file_schema.metadata.insert(
        ID_DOMAIN_META_KEY.to_string(),
        ID_DOMAIN_ROW_ADDR.to_string(),
    );
...
impl BTreeTrainingRequest {
    pub fn new(parameters: BTreeParameters) -> Self {
        Self {
            ...
            // physical row addresses rather than row ids (see
            // `ID_DOMAIN_META_KEY`), so an FRI can repair them after a rewrite.
