...
impl LogicalPageScheduler for ListPageScheduler {
    fn schedule_range(
        ...
        _sink: &mpsc::UnboundedSender<Box<dyn LogicalPageDecoder>>,
    ) -> Result<()> {
        ...
        let indirect_fut = tokio::task::spawn(async move {
            ...
            while let Some(item_decoder) = rx.blocking_recv() {
        ...
        tx.send(Box::new(ListPageDecoder {
            offsets: None,
...
impl LogicalPageDecoder for ListPageDecoder {
    fn wait<'a>(
        ...
        async move {
            ...
            while remaining > 0 {
                let mut next_to_await = self.unawaited.pop_front().ok_or_else(|| Error::Internal { message: format!("list page was asked for {} rows but ran out of item pages before that happened", remaining).into(), location: location!() })?;
