...
impl StructuralFieldDecoder for StructuralListDecoder {
    ...
    fn plan_decoded_bytes(&self, rows_remaining: u64) -> Result<[u64; 8]> {
        // The repdef encodes how many items belong to each list row; without
        // decoding we cannot compute the exact item count. We delegate to the
        // child decoder, which uses the schema-based estimate as a fallback.
        // This is a known limitation and may be improved in future work.
