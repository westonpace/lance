...
impl ValueEncoder {
    fn encode_flat(&self, arrays: &[ArrayRef]) -> Result<EncodedBuffer> {
        let parts = arrays
            .iter()
    ...
    fn encode_fsl(&self, arrays: &[ArrayRef]) -> Result<EncodedBuffer> {
        let parts = arrays
            .iter()
            .map(|arr| arr.as_fixed_size_list().values().to_data().buffers()[0].clone())
            .collect::<Vec<_>>();
        Ok(EncodedBuffer {
            is_data: true,
            parts,
...
impl BufferEncoder for ValueEncoder {
    fn encode(&self, arrays: &[ArrayRef]) -> Result<EncodedBuffer> {
        match self.data_type {
            DataType::FixedSizeList(_, _) => self.encode_fsl(arrays),
            _ => self.encode_flat(arrays),
        }
    }
}
...
mod tests {
    ...
    async fn test_value_fsl_primitive() {
        for data_type in PRIMITIVE_TYPES {
            ...
            let inner_field = Field::new("item", data_type.clone(), false);
            let field = Field::new(
                "",
