...
mod tests {
    ...
    async fn test_round_trip() {
        ...
        let reader = gen()
            ...
            .into_reader_rows(RowCount::from(1000), BatchCount::from(10000));
        ...
        println!("Wrote file in {} seconds", start.elapsed().as_secs_f64());
        ...
        for read_size in [32, 1024, 1024 * 1024] {
            ...
            while let Some(actual) = batch_stream.next().await {
                count += actual.unwrap().num_rows();
            }
            println!(
                "Read in {} rows with read_size={} in {} seconds",
                count,
                read_size,
